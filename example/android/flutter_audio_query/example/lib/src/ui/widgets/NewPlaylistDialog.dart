        ...
        StreamBuilder<PlaylistInfo>(
            ...
            builder: (context, snapshot) {
              return TextButton(
                child: Text('Create'),
                onPressed: () async {
                  await bloc.createPlaylist(_textEditingController!.text, context);
                },
              );
            }),
      ],
...
class PlaylistDialogBloc extends BlocBase {
  ...
  Future<void> createPlaylist(final String playlistName, BuildContext context) async {
    if (playlistName.isEmpty) {
      _errorController.sink.addError('Playlist name is empty!');
    } else {
      await FlutterAudioQuery.createPlaylist(playlistName: playlistName).then((data) {
